...
class EPGController:
    ...
    def _update_epg_display(self):
        ...
        channel_data = self.epg_guide.get_channel_data(self.window.current_channel.epg_id)
        if not channel_data:
            return
        ...
        if current_program:
            ...
            upcoming = [p for p in channel_data.programs if p.start_time > current_time][:5]
